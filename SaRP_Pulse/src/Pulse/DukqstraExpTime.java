...
public class DukqstraExpTime {
	...
	private int iH;
	private int jH;
	public DukqstraExpTime(PulseGraph g, int source) {
		...
		G.getVertexByID(source).setMinExpTime(0);
		Snum = 0;
	...
	/**
	 * Run Dijkstra's algorithm for ExpTime
	 */
	public void runAlgExpTime()
	{
		int numN = numNodes;
		VertexPulse vi;
		int di;
		...
		while (Snum < numN) {
			while (lowIterator < Delta) {
				...
				VertexPulse vj;
				while (e != null) {
//					System.out.println("aca1"+e.getNext());
					vj = e.getTarget();
					tj = vj.getMinExpTime();
					if (tj > ti + e.getWeightExpTime()) {
						int ntj = ti + e.getWeightExpTime();
						// dMap.put(vj.getID(), ndj);
						vj.setMinExpTime(ntj);
						//vj.setMaxDist(di + e.getWeightDist());
						if (ntj < dK) {
							if (vj.isInsertedExpTime()) {
								moveToLow(tj, ntj, vj);
							} else {
								vj.setInsertedExpTime();
								insertLow(ntj, vj);
							}
						} else {
							if (vj.isInsertedExpTime()) {
	...
	private void stepForwardInExpTime() {
		if (baseAproxBucket!=null) {
			indexL= baseAproxBucket.getKey();
			dL = indexL*Delta;
			VertexPulse entrance = baseAproxBucket.getEntrance();
			
			if (entrance.getBLeftExpTime().id==364) {
//				System.out.println("Es2: "+entrance.getBLeftExpTime().id+" "+entrance.id+" "+entrance.getBRigthExpTime().id);
				entrance=entrance.getBLeftExpTime();
				entrance.setLeftExpTime(entrance);
				System.out.println("Es1: "+entrance.getBLeftExpTime().id+" "+entrance.id+" "+entrance.getBRigthExpTime().id);
			}
			entrance.getBLeftExpTime().unlinkRighBoundExpTime();
			...
			int p=0;
			while(entrance!=null){
				baseAproxBucket.deleteToPassExpTime(entrance);
				dv= entrance.getMinExpTime();
				lowLevelBuckets[dv-dL].insertVertexExpTime(entrance);
				entrance = baseAproxBucket.getEntrance();
				p++;
			}
	...
	private void insertLow(int ndj, VertexPulse v){
		lowLevelBuckets[ndj - dL].insertVertexExpTime(v);
	}
	
	private void moveToLow(int dj, int ndj, VertexPulse v){
		if(dj<dK){
			lowLevelBuckets[dj - dL].deleteToMoveExpTime(v);
			insertLow(ndj, v);
		}else{
	...
	private void insert(int key, int clueKey, int ndj,  VertexPulse v )
	{
		...
		if(highLevelBuckets[pos] != null){
			highLevelBuckets[pos].insertVertexExpTime(v);
		}else{
	...
	private boolean deleteToLabel(int lowIter){
		return lowLevelBuckets[lowIter].deleteLabeledVertexExpTime();
	}
	...
	private boolean deleteToMove(int key , VertexPulse v){
		...
		return b.deleteToMoveExpTime(v);
	}
